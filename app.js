...
function updateVacacionesDiasContador() {
    ...
    const tipoTurno = document.getElementById('turno-tipo').value;
    const tooltipBase = 'Días naturales en el rango de fechas seleccionado. Tenga en cuenta el límite anual configurado para el personal.';
    contadorEl.title = tooltipBase;
...
function updateCarpetaDiasContador() {
    ...
    if (!contadorEl) return;
    contadorEl.title = 'Días naturales entre fecha de inicio y fecha de alta de la carpeta médica.';
...
function updateArticulo26Counter() {
    ...
    // Cambiar color si se acerca al límite y tooltip informativo
    const contadorElement = document.getElementById('articulo26-contador');
    if (contadorElement) {
        contadorElement.title = 'Artículo 26: máximo 6 días al año, 1 por mes. Este contador muestra los días utilizados en el año y si ya usó un día en el mes actual.';
        if (diasUtilizados >= 6) {
...
function setupEventListeners() {
    ...
    // Atajos de teclado: ←/→ cambiar mes, Escape cerrar modal o menú
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape') {
            ...
            if (openModal) {
                ...
                if (openModal.id === 'stats-section') cleanupStatsModal();
                e.preventDefault();
            }
            return;
        }
    ...
    // Navegación por teclado en la tabla del calendario (flechas + Enter)
    document.addEventListener('keydown', (e) => {
        const td = e.target;
        if (td.tagName !== 'TD' || !td.closest('#calendar-table')) return;
        ...
        if (e.key === 'ArrowRight' && cellIndex + 1 < tr.cells.length) {
            tr.cells[cellIndex + 1].focus();
            e.preventDefault();
        } else if (e.key === 'ArrowLeft' && cellIndex > 0) {
            tr.cells[cellIndex - 1].focus();
            e.preventDefault();
        } else if (e.key === 'ArrowDown' && rowIndex + 1 < rows.length) {
            ...
            const day = td.dataset.dayIndex;
            const personalId = tr.dataset.personalId;
            if (day && personalId && isAdmin()) {
                const dateStr = formatDate(new Date(currentYear, currentMonth, parseInt(day, 10)));
                const turno = (turnos[dateStr] && turnos[dateStr][personalId]) || null;
                openTurnoModal(personalId, dateStr, turno);
            }
            e.preventDefault();
        }
    });
    ...
    // Leyenda de códigos del calendario
...
function renderStatsDates() {
    ...
    if (!selectedId) {
        ...
        info.className = 'stats-dates-no-select';
    ...
    title.className = 'stats-dates-title';
    ...
    if (types.every(t => fechasPorTipo[t].length === 0)) {
        ...
        empty.className = 'stats-dates-empty-msg';
    ...
    filterWrap.className = 'stats-dates-filter-wrap';
    const filterLabel = document.createElement('label');
    filterLabel.htmlFor = 'stats-dates-filter';
    filterLabel.className = 'stats-dates-filter-label';
    filterLabel.textContent = 'Filtrar por fecha o mes:';
    const filterInput = document.createElement('input');
    filterInput.type = 'text';
    filterInput.id = 'stats-dates-filter';
    filterInput.className = 'stats-dates-filter-input';
    filterInput.placeholder = 'Ej: enero, 05/01, 26';
    ...
    types.forEach(tipo => {
        ...
        block.className = 'stats-dates-type-block';
        container.appendChild(block);

        const toggleBtn = document.createElement('button');
        toggleBtn.type = 'button';
        toggleBtn.className = 'stats-dates-toggle-btn';
        ...
        content.className = 'stats-dates-content';
    ...
    filterInput.addEventListener('input', () => {
        ...
        const monthBlocks = container.querySelectorAll('.stats-dates-month-block');
        pills.forEach(pill => {
            const match = !q || pill.dataset.search.indexOf(q) !== -1;
            pill.classList.toggle('stats-date-pill-hidden', !match);
        });
        monthBlocks.forEach(block => {
            const visibleCount = block.querySelectorAll('.date-pill:not(.stats-date-pill-hidden)').length;
            block.classList.toggle('stats-month-block-hidden', visibleCount === 0);
        });
    });
}
...
function renderPersonalList() {
    ...
    orderedPersonal.forEach(persona => {
        ...
        const vacTooltip = totalVac != null
            ? `Vacaciones: ${usadosVac} de ${totalVac} días usados, ${restoVac} restantes en ${yActual}.`
            : 'Vacaciones: sin límite configurado.';
        const estTooltip = totalEst != null
            ? `Estrés: ${usadosEst} de ${totalEst} días usados, ${restoEst} restantes en ${yActual}.`
            : 'Estrés: sin límite configurado.';
        const veTitle = `${vacTooltip} ${estTooltip}`;
        const vacNearLimit = totalVac != null && totalVac > 0 && (restoVac <= 2 || usadosVac >= totalVac * 0.9);
        const vacAtLimit = totalVac != null && restoVac === 0;
        const estNearLimit = totalEst != null && totalEst > 0 && (restoEst <= 2 || usadosEst >= totalEst * 0.9);
        const estAtLimit = totalEst != null && restoEst === 0;
        const vacItemClass = 've-item' + (vacAtLimit ? ' ve-item-at-limit' : (vacNearLimit ? ' ve-item-near-limit' : ''));
        const estItemClass = 've-item' + (estAtLimit ? ' ve-item-at-limit' : (estNearLimit ? ' ve-item-near-limit' : ''));

        row.innerHTML = `
            <td title="Nombre del personal">${persona.nombre}</td>
            <td title="Apellido del personal">${persona.apellido}</td>
            <td title="Días de trabajo y horario (o SADOFE)">${daysLabel}${ws && ws.type === 'fixed' ? ` · ${timeLabel}` : (ws && ws.type === 'per_day' ? ' · Por día' : '')}</td>
            <td title="Turno preferente (día o noche)">${persona.turnoPreferente === 'noche' ? 'Noche' : 'Día'}</td>
            <td class="ve-cell" title="${veTitle.replace(/"/g, '&quot;')}">
                <div class="ve-stats">
                    <div class="${vacItemClass}" title="${vacTooltip.replace(/"/g, '&quot;')}">
                    ...
                    <div class="${estItemClass}" title="${estTooltip.replace(/"/g, '&quot;')}">
...
/** Día de la semana (0=domingo .. 6=sábado) para un día del calendario. Usa mediodía local para evitar problemas de zona horaria/DST. */
function getDayOfWeekForCalendar(year, month, day) {
    const d = new Date(year, month, day, 12, 0, 0, 0);
    return d.getDay();
}

/** true si el día es sábado (6) o domingo (0). */
...
function renderCalendar() {
    ...
    const isCurrentMonthHeader = today.getFullYear() === currentYear && today.getMonth() === currentMonth;
    const todayDayHeader = isCurrentMonthHeader ? today.getDate() : 0;

    // Orden del personal (necesario para calcular días sin guardia)
    ...
    // Días del mes en que nadie tiene guardia (G o CG)
    const daysSinGuardia = new Set();
    for (let day = 1; day <= daysInMonth; day++) {
        const dateStr = formatDate(new Date(currentYear, currentMonth, day));
        const dayTurnos = turnos[dateStr] || {};
        const hasGuardia = orderedPersonal.some(p => {
            const t = dayTurnos[p.id];
            return t && (t.tipo === 'guardia_fija' || t.tipo === 'cambios_guardia');
        });
        if (!hasGuardia) daysSinGuardia.add(day);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (day === todayDayHeader) {
            th.classList.add('calendar-day-today');
            th.title = (th.title ? th.title + '. ' : '') + 'Hoy';
        }
        if (daysSinGuardia.has(day)) {
            th.classList.add('calendar-day-no-guardia');
            th.title = (th.title ? th.title + '. ' : '') + 'Sin guardia este día';
        }
    ...
    // Día de hoy para resaltar columna en el cuerpo (ya definido today/todayDayHeader arriba)
    ...
    orderedPersonal.forEach(persona => {
        ...
        // Columna con nombre del personal (tabindex para navegación por teclado)
        const nameCell = document.createElement('td');
        nameCell.classList.add('name-cell');
        nameCell.setAttribute('tabindex', '-1');
        nameCell.textContent = `${persona.apellido}, ${persona.nombre}`;
        const personColor = (getSettings().personColors || {})[persona.id];
        if (personColor) {
            nameCell.style.borderLeft = `4px solid ${personColor}`;
            nameCell.title = 'Color asignado en Configuraciones';
        }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const dateStr = formatDate(new Date(currentYear, currentMonth, day));
            // Marcar fin de semana (sábado/domingo) y feriados con cálculo estable a zona horaria
            if (isWeekendDay(currentYear, currentMonth, day) || isHoliday(currentYear, currentMonth, day)) {
            ...
            if (daysSinGuardia.has(day)) {
                cell.classList.add('calendar-day-no-guardia');
            }
...
function goToToday() {
    ...
    if (container) {
        const todayTh = document.querySelector('#calendar-table thead th.calendar-day-today');
        if (todayTh) {
            const left = todayTh.offsetLeft - container.clientWidth / 2 + todayTh.offsetWidth / 2;
...
function openHelpModal() {
    ...
    const listEl = document.getElementById('help-codes-list');
    if (!modal || !listEl) return;
    if (listEl.children.length === 0) {
        const baseTypes = ['guardia_fija', 'ausente', 'vacaciones', 'compensatorio', 'estres', 'cambios_guardia', 'articulo26', 'carpeta_medica', 'dia_sindical', 'dia_estudio'];
        const s = getSettings();
        const customTypes = (s && Array.isArray(s.customTypes)) ? s.customTypes.map(ct => ct.id) : [];
        const allTypes = baseTypes.concat(customTypes);
...
function handleTurnoSubmit(e) {
    ...
    switch(tipo) {
        ...
        case 'estres': {
            ...
            if (!conditionalData.fechaInicio || !conditionalData.fechaFin) {
                showNotification(tipo === 'vacaciones'
                    ? 'Indique la fecha de inicio y de finalización para las vacaciones.'
                    : 'Indique la fecha de inicio y de finalización para los días de estrés.');
                isSubmitting = false;
                return;
            }
            const sV = parseDateLocal(conditionalData.fechaInicio);
            const eV = parseDateLocal(conditionalData.fechaFin);
            if (!sV || !eV || sV > eV) {
                showNotification('La fecha de inicio debe ser anterior o igual a la fecha de finalización.');
                isSubmitting = false;
                return;
            }
            break;
        ...
        case 'carpeta_medica': {
            ...
            if (!conditionalData.fechaInicioCarpeta || !conditionalData.fechaAlta) {
                showNotification('Indique la fecha de inicio y la fecha de alta de la carpeta médica.');
                isSubmitting = false;
                return;
            }
            const sC = parseDateLocal(conditionalData.fechaInicioCarpeta);
            const eC = parseDateLocal(conditionalData.fechaAlta);
            if (!sC || !eC || sC > eC) {
                showNotification('La fecha de inicio debe ser anterior o igual a la fecha de alta.');
                isSubmitting = false;
                return;
            }
            break;
            ...
            if (typeof personaInfo.diasVacaciones === 'number' && personaInfo.diasVacaciones >= 0) {
                ...
                for (const yStr of Object.keys(newDaysByYear)) {
                    ...
                    if (proposed > personaInfo.diasVacaciones) {
                        const restantes = Math.max(0, personaInfo.diasVacaciones - used);
                        showNotification(`Límite de vacaciones alcanzado para ${y}. Máximo: ${personaInfo.diasVacaciones} días. Quedan ${restantes} disponibles. Reduzca el rango de fechas.`);
                        isSubmitting = false;
                        return;
                    }
                }
            ...
            if (typeof personaInfo.diasEstres === 'number' && personaInfo.diasEstres >= 0) {
                ...
                for (const yStr of Object.keys(newDaysByYear)) {
                    ...
                    if (proposedDays > personaInfo.diasEstres) {
                        const restantesEst = Math.max(0, personaInfo.diasEstres - usedDays);
                        showNotification(`Límite de estrés alcanzado para ${y}. Máximo: ${personaInfo.diasEstres} días. Quedan ${restantesEst} disponibles. Reduzca el rango de fechas.`);
                        isSubmitting = false;
                        return;
                    }
                }
...
// Utilidad para animar elementos al agregarlos al DOM
function animateIn(element, animationClass = 'fadeInUp') {
    element.style.opacity = '0';
    requestAnimationFrame(() => {
        element.style.animation = `${animationClass} 0.4s ease-out forwards`;
...
/** Construye texto para tooltip de un turno (horario, observaciones). */
function buildTurnoTooltipText(turno, tipo) {
    if (!turno) return '';
    const parts = [];
    const label = getTypeLabel(tipo);
    if (label) parts.push(label);
    if (turno.horaEntrada && turno.horaSalida) {
        parts.push(`Horario: ${turno.horaEntrada} - ${turno.horaSalida}`);
    }
    if (turno.observaciones && String(turno.observaciones).trim()) {
        const obs = String(turno.observaciones).trim();
        parts.push(obs.length > 80 ? obs.slice(0, 77) + '…' : obs);
    }
    return parts.join(' · ');
}
            ...
            settingsForm.addEventListener('submit', (e) => {
                ...
                showConfirmModal({
                    ...
                    onAccept: async () => {
                        ...
                        newS.personColors = getPersonColorsFromForm();
                        saveSettings(newS);
                        addMovementLog({
...
function renderPersonColorsList(s) {
    const list = document.getElementById('person-colors-list');
    if (!list) return;
    list.innerHTML = '';
    const personColors = (s && s.personColors) ? s.personColors : {};
    personal.forEach(p => {
        const row = document.createElement('div');
        row.className = 'person-color-row';
        row.dataset.personId = p.id;
        const label = document.createElement('span');
        label.className = 'person-color-label';
        label.textContent = `${p.apellido}, ${p.nombre}`;
        const colorInp = document.createElement('input');
        colorInp.type = 'color';
        colorInp.value = personColors[p.id] || '#3b82f6';
        colorInp.className = 'person-color-input';
        colorInp.title = 'Clic para elegir color (se muestra como borde izquierdo en la fila del calendario)';
        const swatch = document.createElement('div');
        swatch.className = 'person-color-swatch';
        swatch.style.backgroundColor = colorInp.value;
        swatch.appendChild(colorInp);
        colorInp.addEventListener('input', () => { swatch.style.backgroundColor = colorInp.value; });
    ...
    if (personal.length === 0) {
        ...
        empty.className = 'person-colors-empty';
        empty.textContent = 'No hay personal. Agregue personal en Gestionar Personal.';
...
function getPersonColorsFromForm() {
    const list = document.getElementById('person-colors-list');
    if (!list) return {};
    const out = {};
    list.querySelectorAll('.person-color-row').forEach(row => {
        const id = row.dataset.personId;
        const inp = row.querySelector('.person-color-input');
        if (id && inp && inp.value) out[id] = inp.value;
    });
    return out;
}
    ...
    if (stList) {
        ...
        const renderSTEdit = (index) => {
            ...
            const createColorPicker = (label, prop, defaultVal) => {
                ...
                // Si el clic cae en el wrapper o el swatch, abrir la paleta (evita que a veces no abra)
    ...
    if (list) {
        ...
        const CustomTypeManager = {
            ...
            renderEditForm: (index) => {
                ...
                const typeIdForInput = (typeData.id || 'custom').replace(/[^a-z0-9_-]/gi, '_');
                const createColorPicker = (label, prop, defaultVal) => {
                    const container = document.createElement('div');
                    container.className = 'color-picker-group';
                    ...
                    input.id = `ct-color-${typeIdForInput}-${prop}`;
